...
function App() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const slug =
        form.appName.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") ||
        "app";
      a.href = url;
      a.download = `${slug}-prd.zip`;
  ...
  return (
    <div className="page">
      <header className="page-header">
        <h1>PRD Builder</h1>
        <p>
          Answer a few questions about your app idea. We'll generate a PRD and a ready-to-use
          Claude Code prompt.
        </p>
      </header>
      ...
      <form className="prd-form" onSubmit={handleSubmit}>
        ...
        <div className="submit-row">
          <button type="submit" disabled={status === "submitting"}>
            {status === "submitting" ? "Generating..." : "Generate PRD"}
          </button>
          {status === "error" && <span className="error-msg">{error}</span>}
          {status === "done" && <span className="success-msg">Downloaded!</span>}
        </div>
      </form>
    </div>
  );
